<script lang="ts">
	...
	import AccurateBeeswarm from '../ts/accurate-beeswarm';
	...
	function setCoords(data, custom, x, y, r, width) {
		...
		if (type == 'bar') {
			let xpos: number[] = [];
			let xneg: number[]  = [];
			newcoords = data.map((d, i: number) => {
				return d.map((e, j: number) => {
					if (!xpos[j]) xpos[j] = 0;
					if (!xneg[j]) xneg[j] = 0;
					let x0 =
						mode == 'default' || mode == 'grouped' || (mode == 'comparison' && i == 0)
							? $xDomain[0]
							: mode == 'stacked' && x(e) >= 0
							? xpos[j]
							: mode == 'stacked'
					...
					let y0 =
						mode == 'grouped' ? $yGet(e) + i * ($yScale.bandwidth() / data.length) : $yGet(e);
					let y1 =
						mode == 'grouped' ? y0 + $yScale.bandwidth() / data.length : y0 + $yScale.bandwidth();
					return { x0, x1, y0, y1 };
				});
			});
		} else if (type == 'column') {
			...
			newcoords = y
				...
				: new AccurateBeeswarm(
						...
						.calculateYPositions()
						.map((d) => ({ x: $xScale.invert(d.x), y: $yScale.invert(d.y), r: d.r }));
		} else {
